...
var Reporter = /** @class */ (function () {
    ...
    Reporter.prototype.screenshotPath = function () {
        const initialRoute = Constants.REPORT_PATH;
        console.log("Esta es la ruta inicial: " + initialRoute);
    
        let folderCount = 0;
    
        try {
            const items = fs.readdirSync(initialRoute, { withFileTypes: true });
            folderCount = items.filter((item) => item.isDirectory()).length;
        } catch (error) {
            console.error("Error al leer el directorio:", error);
        }
        console.log("Cantidad de carpetas en la ruta inicial:", folderCount);

        const folderName = folderCount + 1;
        console.log("Nombre de la carpeta para screenshots:", folderName);

        return (
            Constants.REPORT_PATH +
            "/" +
            this.testScenario.executionId +

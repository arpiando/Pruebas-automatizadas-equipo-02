...
async function recursiveExploration(page, link, depth, parentState){
  ...
  try {
    ...
    if (link.includes('/signin') || (depth == 0)){
      await page.waitForSelector('input[type="email"]');

      await page.fill('input[type="email"]', inputValues[4]); 
      await page.fill('input[type="password"]', inputValues[1]);
    ...
    // Captura el error aquí
    console.log("Error al navegar a la página:", err);

    // Guarda una captura de pantalla para el error
    let err_name = err.toString();
    let capture_path = screenshots_directory + err_name + '.png';
    await page.screenshot({path: capture_path});

    // Puedes agregar más lógica aquí para manejar el error, 
    // como registrar el error o intentar cargar la página de nuevo.

    return; // Sale de la función para continuar con la siguiente iteración
}
